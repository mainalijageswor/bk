...
export const makesPenalty = async (req, res) => {
  try {
    let np = new NepaliDate().getBS();
    let year = np.year;
    let month = np.month;
    let day = np.date;
    np = new NepaliDate(year, month, day).format("YYYY-MM-DD");
    const booksTogetPenalty = await prisma.bookTransaction.findMany({
      where: {
        Status: "Issued",
        AND: {
          returnedAt: {
    ...
    if (booksTogetPenalty.length > 0) {
      booksTogetPenalty.forEach(async (book) => {
        let Fine_Amount;
        let AdDueDate;
        let AdToday;
        ...
        if (differenceInDays > 0) {
          ...
          const FinedTransaction = await prisma.bookTransaction.update({
            where: {
              Id: Number(book.Id),
            },
            data: {
...
export const getStudentTransaction = async (req, res) => {
  try {
...
export const getStudentDashboardInfo = async (req, res) => {
  try {
    const { Student_ID } = req.body;
    let today = new Date();
    today = `${today.getFullYear()}-${today.getMonth() + 1}-${today.getDate()}`;
