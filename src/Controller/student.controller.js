...
const fs = require("fs");
const deleteResources = async (filePath) => {
...
export const processLogin = async (req, res) => {
  try {
    ...
    const isStudentValid = await prisma.students.findUnique({
      where: {
        Email: email,
      },
    });
...
export const StudentPassAvatarChange = async (req, res) => {
  try {
...
export const userProfilePicUpdate = async (req, res) => {
  try {
    const { id } = req.body;
    const uploadedPhoto = req.file?.path;
    const user = await prisma.students.findUnique({
...
export const sendOTPPRESET = async (req, res) => {
  try {
    const { email } = req.body;
    const isStudentFound = await prisma.students.findUnique({
      where: {
...
export const newPasswordReq = async (req, res) => {
  try {
    const { id, password } = req.body;
    const hashedPassword = await bcrypt.hash(
      password,
